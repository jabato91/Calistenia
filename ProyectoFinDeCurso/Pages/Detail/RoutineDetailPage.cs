            ...
            editRoutine.GestureRecognizers.Add(new TapGestureRecognizer
            {
                Command = new Command(async () =>
                {
                    ...
                    var bodyPartEnumPicker = new Picker
                    {
                        ...
                        SelectedItem = translation[_routine.muscleGroup],
                        TextColor = Color.FromArgb("#C49362"),
                        BackgroundColor = Color.FromArgb("#3B2523"),
                    ...
                    ObservableCollection<Exercise> exercisesInRoutine = new ObservableCollection<Exercise>();
                    foreach (Exercise exercise in _routine.Exercises)
                    {
                        exercisesInRoutine.Add(exercise);
                    }
                             ...
                             new Button
                        {
                            ...
                            Command = new Command(async () =>
                            {
                                ...
                                else
                                {
                                    ...
                                    _routine.nameRoutine = nameRoutineEntry.Text;
                                    _routine.description = DescriptionRoutineEntry.Text;
                                    _routine.muscleGroup = selectedEnum;
                                    _routine.Exercises = exercisesInRoutine;


                                    var eliminateExercises = existingRelations.Where(r => r.RoutineID.Equals(_routine.routineID)).ToList();

                                    foreach (var relation in eliminateExercises)
                                    {
                                        if (relation.RoutineID.Equals(_routine.routineID))
                                        {
                                    ...
                                    foreach (Exercise exercise in _routine.Exercises)
                                    {
                                         await _dbService.Create(new RoutinesExercises {RoutineID = _routine.routineID,ExerciseID = exercise.execiseID,sets = exercise.sets, reps = exercise.reps,seconds = exercise.seconds});
                                    }
